...
- `CS-FOUND-02` | status: in_progress | execution: claimed | next: Add only the legacy codec/container baseline tests, run G1 gates, record results, reach G1, and stop without claiming CS-FOUND-03.
- `CS-FOUND-03` | status: planned | execution: ready | next: Implement and validate CS-FOUND-I1 only; do not modify legacy identities.
...
- `CS-FOUND-07` | status: planned | execution: ready | next: Review I1/I2A/I2B/I2C, reach C1 and G2, then implement catalog/snapshot validation without persistent manifest format.
- `CS-FOUND-08` | status: planned | execution: ready | next: Implement fixed-field/table codec, deterministic padding/checksum, size calculation, and malformed-buffer tests only.
- `CS-FOUND-09` | status: planned | execution: ready | next: Add new image store/inspect APIs, expose an extent, validate top-level offsets, and preserve the legacy writer.
- `CS-FOUND-10` | status: planned | execution: ready | next: Review and freeze I3A/I3B/I4 at G3, then implement pread-backed local exact reads and one-allocation host residency.
...
- `CS-FOUND-15` | status: planned | execution: ready | next: Run focused/full feasible CPU and CUDA tests, installed package smoke, static architecture scans, todo audit/reconcile/export, reach G7, and hand off with CS-PLACE recommendation only.
