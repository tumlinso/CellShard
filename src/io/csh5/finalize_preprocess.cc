...
int finalize_preprocessed_blocked_ell_dataset_h5(const char *filename,
    ...
    if (filename == 0 || cell_keep == 0 || gene_keep == 0) return 0;
    ...
    temp_path = (fs::path(filename).parent_path() / (fs::path(filename).filename().string()
        + ".preprocess_finalize." + std::to_string((unsigned long long) ::getpid()) + ".tmp")).string();
    {
        std::error_code ec;
        fs::remove(temp_path, ec);
    }
    if (!create_dataset_optimized_blocked_ell_h5(temp_path.c_str(), &layout, &dataset_view, &provenance_view)) goto done;
    ...
    {
        std::error_code ec;
        fs::rename(temp_path, filename, ec);
        if (ec) goto done;
    }
