# Todo Status

## Summary
Use this file as the quick pickup register for `todos.md` workstreams.
- `ready`: planned work that can be started now.
- `claimed`: currently being written; choose another stream.
- `idle`: unfinished but resumable; safe to pick up.
- `closed`: completed or removed from pickup rotation.

## Workstreams
- `cellshard-cpp-access-adapter-refactor` | status: in_progress | execution: claimed | owner: codex | file: `todos/cellshard-cpp-access-adapter-refactor.md` | next: Implement the CellShard access headers and narrow tests first, then migrate production consumers incrementally.

## Staleness Review
_No staleness review recorded yet._

## Cleanup Status
- Cleanup mode is explicit only.
- Safe to call `todo-cleanup`: no, active workstreams: cellshard-cpp-access-adapter-refactor.
